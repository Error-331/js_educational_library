...
} from './../../../src/utils/hashing/polynomial_rolling_hash_utils.js';
    ...
    await t.test('calcPolynomialRollingHash() function tests...', async (t) => {
        ...
        /*await t.test('case case-insensitive alphanumeric hash calculation tests...', async (t) => {
            await t.test('Should turn string into a hash - case 1...', async () => {
                const hash = calcPolynomialRollingHash( false, HASHING_P_PRIME_NUMBER_ALPHABETIC1, HASHING_M_PRIME_NUMBER1, stringToHash1);
                assert.strictEqual(hash, caseInsensitiveHashResult1);
            });

            await t.test('Should turn string into a hash - case 2...', async () => {
                const hash = calcPolynomialRollingHash( false, HASHING_P_PRIME_NUMBER_ALPHABETIC1, HASHING_M_PRIME_NUMBER1, stringToHash2);
                assert.strictEqual(hash, caseInsensitiveHashResult2);
            });

            await t.test('Should turn string into a hash - case 3...', async () => {
                const hash = calcPolynomialRollingHash( false, HASHING_P_PRIME_NUMBER_ALPHABETIC1, HASHING_M_PRIME_NUMBER1, stringToHash3);
