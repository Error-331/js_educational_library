...
import BinarySearchTreePreOrderIteratorClass from './../../../../src/data_structures/binary_tree/traversal/binary_search_tree_pre_order_iterator_class.js';

// implementation
test('BinarySearchTreePreOrderIteratorClass tests...', async (t) => {
    ...
    await t.test('Should traverse a tree - case 1', () => {
        ...
        BinarySearchTreePreOrderIteratorClass.traverse(treeInstance, (node) => traversedKeys.push(node.key));
        assert.deepStrictEqual(traversedKeys, testSequence1);

        traversedKeys = [];
        const iterator = treeInstance.createPreOrderIterator();
