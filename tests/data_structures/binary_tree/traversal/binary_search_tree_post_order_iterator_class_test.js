...
import BinarySearchTreePostOrderIteratorClass from './../../../../src/data_structures/binary_tree/traversal/binary_search_tree_post_order_iterator_class.js';

// implementation
test('BinarySearchTreePostOrderIteratorClass tests...', async (t) => {
    ...
    await t.test('Should traverse a tree - case 1', () => {
        ...
        BinarySearchTreePostOrderIteratorClass.traverse(treeInstance, (node) => traversedKeys.push(node.key));
        assert.deepStrictEqual(traversedKeys, testSequence1);

        traversedKeys = [];
        const iterator = treeInstance.createPostOrderIterator();
